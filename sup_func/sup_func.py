...
def parse_left_args(args, left, argument_func):
    parser_new = argparse.ArgumentParser(allow_abbrev=False)
...
def printc(content, color='reset'):
    ...
    # print(content)
    # sys.stdout.flush()
    # Disable color if not writing to a terminal (i.e., redirected to file)
    ...
    print(color_prefix + str(content) + color_codes['reset'])
